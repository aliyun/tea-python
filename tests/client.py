...
class Client:
    ...
    async def file_test_async(
        ...
        if DaraFile.exists_async('/tmp/test'):
            file = DaraFile('/tmp/test')
            path = file.path()
            length = await file.length_async() + 10
            create_time = await file.createTime_async()
            modify_time = await file.modifyTime_async()
            time_long = modify_time.diff('minute', create_time)
    ...
    async def stream_test_async(
        ...
        if DaraFile.exists_async('/tmp/test'):
            ...
            data = DaraStream.read_as_bytes_async(rs)
            obj = DaraStream.read_as_json_async(rs)
            json_str = DaraStream.read_as_string_async(rs)
    ...
    def main(
        ...
        c = Number(b) + Number(a) + Number(Client.return_any())
        ...
        DaraCore.sleep(a)
        default_val = str(args[0] or args[1])
        if default_val == b:
            return
